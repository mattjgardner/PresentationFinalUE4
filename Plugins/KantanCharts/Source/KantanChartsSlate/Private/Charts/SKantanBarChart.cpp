...
void SKantanBarChart::Construct(const FArguments& InArgs)
{
	...
	//SetMinBarValue(InArgs._MinBarValue); //this needs to be added to args if widget to be set in construct
...
void SKantanBarChart::SetMinBarValue(float InMin)
{
	MinBarValue = InMin;
}
...
FCartesianAxisRange SKantanBarChart::DetermineValueRange() const
{
	...
	//NOTE set input value for Range.Min
	//Although the rest of the code is KantanDev, this part was developed by me to allow for negative values
	if (MinBarValue != 0)
	{
		Range.Min = MinBarValue;
	}
	else
	{
		Range.Min = 0.0f;
