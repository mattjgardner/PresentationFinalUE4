...
void UWheeledVehicleMovementComponent::SetSuspensionData(float inSuspensionDampingRatio, float inMaxCompression, float inMaxDrop, float inNaturalFrequency)
{
	//Setup for this function adapted from Engine
	
	TArray<PxVec3> WheelOffsets;
	WheelOffsets.Init(PxVec3(), WheelSetups.Num());

	TArray<float> SprungMasses;
	SprungMasses.Init(0.f, WheelSetups.Num());

	PxRigidActor* PActor;

	FPhysicsCommand::ExecuteWrite(UpdatedPrimitive->GetBodyInstance()->ActorHandle, [&](const FPhysicsActorHandle& Actor)
	{
		PActor = FPhysicsInterface::GetPxRigidActor_AssumesLocked(Actor);
		if (!PActor)
		{
			return;
		}
	...
	PxVehicleWheelsSimData PWheelsSimData = PVehicle->mWheelsSimData;
	...
	{
		const PxTransform PLocalCOM = PVehicleActor->getCMassLocalPose();
		PxVehicleComputeSprungMasses(WheelSetups.Num(), WheelOffsets.GetData(), PLocalCOM.p, PVehicleActor->getMass(), /*gravityDirection=*/2, SprungMasses.GetData());
	}
	...
	for (int32 WheelIdx = 0; WheelIdx < NumWheels; WheelIdx++)
	{
		...
		//if data provided = 0, set the value back to the class default

		if (inMaxCompression != 0)
		{
			PSuspensionData.mMaxCompression = inMaxCompression;
		}
		else 
		{
			PSuspensionData.mMaxCompression = Wheel->SuspensionMaxRaise;
		...
		if (inMaxDrop != 0)
		{
			PSuspensionData.mMaxDroop = inMaxDrop;
		}
		else
		{
			PSuspensionData.mMaxCompression = Wheel->SuspensionMaxDrop;
		}

		//Standard cars have values between 5 and 10 so I extended range slightly for more extreme
		if (inNaturalFrequency != 0 && inNaturalFrequency >= 3 && inNaturalFrequency <= 12 )
		{
			PSuspensionData.mSpringStrength = FMath::Square(inNaturalFrequency) * PSuspensionData.mSprungMass;
		}
		else
		{
			PSuspensionData.mSpringStrength = FMath::Square(Wheel->SuspensionNaturalFrequency) * PSuspensionData.mSprungMass;
		...
		//Standard cars have values between 0.8 and 1.2 so range extended slightly for more extreme
		if (inSuspensionDampingRatio != 0 && inSuspensionDampingRatio >= 0.6 && inSuspensionDampingRatio <= 1.4)
		{
			PSuspensionData.mSpringDamperRate = inSuspensionDampingRatio * 2.0f * FMath::Sqrt(PSuspensionData.mSpringStrength * PSuspensionData.mSprungMass);
		}
		else
		{
			PSuspensionData.mSpringDamperRate = Wheel->SuspensionDampingRatio * 2.0f * FMath::Sqrt(PSuspensionData.mSpringStrength * PSuspensionData.mSprungMass);
...
void UWheeledVehicleMovementComponent::SetWheelData(float inRadius, float inWidth, float inMaxSteerAngle, float inMass, float inDampingRate)
{
	...
	for (int32 WheelIdx = 0; WheelIdx < NumWheels; ++WheelIdx)
	{
		...
		//Sets radius to user specification, if user doesn't set, it defaults to the static class default (constructor values)

		if (inRadius != 0)
		{
			PWheelData.mRadius = inRadius;
		}
		else
		{
			PWheelData.mRadius = Wheel->ShapeRadius;
		...
		if (inMaxSteerAngle != 0 && inMaxSteerAngle <= 70)
		{
			PWheelData.mMaxSteer = WheelSetups[WheelIdx].bDisableSteering ? 0.f : FMath::DegreesToRadians(inMaxSteerAngle);
		}
		else
		{
			PWheelData.mMaxSteer = WheelSetups[WheelIdx].bDisableSteering ? 0.f : FMath::DegreesToRadians(Wheel->SteerAngle);
		...
		if (inDampingRate != 0)
		{
			PWheelData.mDampingRate = M2ToCm2(inDampingRate);
		}
		else
		{
			PWheelData.mDampingRate = M2ToCm2(Wheel->DampingRate);
		...
		//Sets values that user can't change to default values
		PWheelData.mMaxBrakeTorque = M2ToCm2(Wheel->MaxBrakeTorque);
		PWheelData.mMaxHandBrakeTorque = Wheel->bAffectedByHandbrake ? M2ToCm2(Wheel->MaxHandBrakeTorque) : 0.0f;
		PWheelData.mMOI = 0.5f * PWheelData.mMass * FMath::Square(PWheelData.mRadius);
...
//Adapted from VehicleWheel.cpp
FPhysXVehicleManager* UWheeledVehicleMovementComponent::GetVehicleManager() const
{
	UWorld* World = GetWorld();
	if (World)
	{
...
//Adapted from VehicleWheel.cpp
float UWheeledVehicleMovementComponent::GetSuspensionOffset(int32 inWheelIndex) const
{

	#if WITH_PHYSX_VEHICLES
	if (FPhysXVehicleManager* VehicleManager = GetVehicleManager())
	{
		return (VehicleManager->GetWheelsStates_AssumesLocked(this)[inWheelIndex].suspJounce);
	}
	#endif // WITH_PHYSX
...
float UWheeledVehicleMovementComponent::GetMaxSuspensionCompression(int32 inWheelIndex)
{
	return PVehicle->mWheelsSimData.getSuspensionData(inWheelIndex).mMaxDroop;
}
